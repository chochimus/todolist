class Todo:
    ...
    def __str__(self):
        marker = Todo.COMPLETE_MARKER if self.done else Todo.INCOMPLETE_MARKER
...
class TodoList:
    ...
    def add(self, todo):
        if not isinstance(todo, Todo):
            raise TypeError("Must be a Todo object.")
        
        self._todos.append(todo)

    def __str__(self):
        result = [f"----- Today's Todos -----"]
        result += [str(todo) for todo in self._todos]
        return '\n'.join(result)
